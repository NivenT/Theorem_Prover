...
(defun simplify-helper (expr rules)
	...
	(let ((bindings (pick-bindings (strip (pat-match '(or ?a.. (and ?b (not ?c)) ?d.. (and ?c (not ?b)) ?e..) expr)))))
		(when (consistent-bindingsp bindings)
			(return-from simplify-helper (list* 'or (list 'xor (sublis bindings '?b) (sublis bindings '?c)) (bind bindings '(?a.. ?d.. ?e..))))))
	expr)
