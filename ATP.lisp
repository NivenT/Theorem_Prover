...
(defun simplify-helper (expr rules)
	...
	(let ((bindings (pick-bindings (strip (pat-match '(and ?a.. t ?b..) expr)))))
		(when (consistent-bindingsp bindings)
			(return-from simplify-helper (list* 'and (apply #'append (remove-if #'variablep (sublis bindings '(?a.. ?b..))))))))
	(let ((bindings (pick-bindings (strip (pat-match '(or ?a.. f ?b..) expr)))))
		(when (consistent-bindingsp bindings)
			(return-from simplify-helper (list* 'or (apply #'append (remove-if #'variablep (sublis bindings '(?a.. ?b..))))))))
	expr)
...
(defun resolve (expr1 expr2)
	(let ((bindings (pick-bindings (strip (pat-match '(and (or ?a.. ?x ?b..) (or ?c.. (not ?x) ?d..)) `(and ,expr1 ,expr2))))))
		(when (consistent-bindingsp bindings)
			(return-from resolve (list* 'or (apply #'append (remove-if #'variablep (sublis bindings '(?a.. ?b.. ?c.. ?d..))))))))
	(let ((bindings (pick-bindings (strip (pat-match '(and (or ?a.. (not ?x) ?b..) (or ?c.. ?x ?d..)) `(and ,expr1 ,expr2))))))
		(when (consistent-bindingsp bindings)
			(return-from resolve (list* 'or (apply #'append (remove-if #'variablep (sublis bindings '(?a.. ?b.. ?c.. ?d..))))))))
	)
(defun prove (conclusion &rest given)
	(refute (append given `((not ,conclusion)))))
(defun temp (statement)
	(cond	((matches '(or ?..) statement) statement)
			(t `(or ,statement f))))
(defun refute (statements &optional (res (car (last statements))))
	(if (equal res 'f) 
		(append statements (list 'qed))
		(progn
			(loop for s in statements do
				(let ((r (simplify (resolve (temp res) (temp s)))))
					(when r (return-from refute (refute (append statements (list r)) r)))))
