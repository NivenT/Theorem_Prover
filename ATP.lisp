...
(defun prove-helper (conclusion given)
	(refute (append (mapcar #'(lambda (f) (list f 'g)) given) `(((not ,conclusion) n)))))
	...
	(if (equal res 'f) 
		...
		(progn
			(loop for s in statements for i from 1 do
				(let ((r (simplify (resolve (temp res) (temp (car s))))))
					(when r (return-from refute (refute (append statements (list (list r i))) r)))))
