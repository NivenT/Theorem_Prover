...
(defun resolve (expr1 expr2)
	...
	(let ((res (transform `(and ,(to-or (->cnf expr1)) ,(to-or (->cnf expr2))) *resolution-rules* :rewrite #'absorb-args)))
		(unless (equalp res `(and ,(to-or (->cnf expr1)) ,(to-or (->cnf expr2)))) res))
	)
(defun split-and (wff)
...
(defparameter *->cnf-rules* '(	((not (and ?a ?*)) -> (or (not ?a) (not (and ?*))))
								...
								;required because of "equiv" complications
								((or (and ?a (not ?b)) (and ?b (not ?a))) -> (xor ?a ?b))
								))
(defun ->cnf (expr)
